model = dict(
    type = 'RecognizerViseceITM',
    itm_dim = 768, # 数值上与video_encoder和text_encoder的output_channels维度一致
    video_encoder = dict(
        type = 'Visece_Video',
        res_out_channels = 512, # ResNet输出的维度
        output_channels = 768, # video_encoder输出的维度
    ...
    text_lists = 'lists/ntu_60_labels.csv'
)
dataset_type = 'PoseDataset'
ann_file = '/home/yl/public_datasets/heatmap/ntu60_hrnet.pkl'
...
optimizer = dict(
    ...
    paramwise_cfg = dict(
        custom_keys = {
            'video_encoder': dict(lr_mult = 1, decay_mult = 0.9),
            'text_encoder': dict(lr_mult = 0.01, decay_mult = 0.9),
            'tokenizer': dict(lr_mult = 0.01, decay_mult=0.9)
        }
    )
)
...
log_level = 'INFO'
work_dir = './work_dirs/plus/ntu-xview-itm'
