...
data = dict(
    ...
    train=dict(
        type='RepeatDataset',
        times=10,
        dataset=dict(type=dataset_type, ann_file=ann_file, split='train2', pipeline=train_pipeline)),
    val=dict(type=dataset_type, ann_file=ann_file, split='test2', pipeline=val_pipeline),
    test=dict(type=dataset_type, ann_file=ann_file, split='test2', pipeline=test_pipeline))
# optimizer
...
log_level = 'INFO'
work_dir = './work_dirs/plus/hmdb-split2-poseconv3d'
